...
public class HotelReservation {
    ...
    public void isEmptyOrNull(String data) throws CustomerException {
        if (data == null)
            throw new CustomerException(CustomerException.ExceptionType.ENTERED_NULL,
                    "Data can't be null");
        if (data.length() == 0)
            throw new CustomerException(CustomerException.ExceptionType.ENTERED_EMPTY,
                    "Data can't be empty");
    }

    // Checking the validity of date format
    public boolean isDateValid(String date) throws CustomerException {
        isEmptyOrNull(date);
        Pattern pattern = Pattern.compile(Date_Pattern);
        if (!pattern.matcher(date).matches())
            throw new CustomerException(CustomerException.ExceptionType.ENTERED_INVALID,
                    "Enter valid date");
        return true;
    }

    // Checking the validity of customer type
    public boolean isCustomerTypeValid(String customerType) throws CustomerException {
    ...
    public LocalDate formatDate(String date) throws CustomerException {
        isDateValid(date);
    ...
    public int calculateNumberOfDays(String startDate, String endDate) throws CustomerException {
    ...
    public int noOfWeekends(String date) throws CustomerException {
        int count = 0;
    ...
    public int calculatePrice(Hotel hotel, String customerType) throws CustomerException {
